  ...
  return (
    <main onClick={(e) => e.stopPropagation()} className='fixed w-[100svw] cursor-pointer h-[100dvh] top-0 left-0 z-[99999999999999999] pointer-none: bg-[rgba(0,0,0,0.7)]'>
        ...
        <div className='flex flex-col'>
            ...
            <button onClick={(e)=>{e.stopPropagation();setEpStatus(tempEpStatus);setIsOpen(false);}} className='text-white right-5 cursor-pointer hover:text-gray-300 self-start bg-themeDark min-w-7 min-h-7 flex justify-center items-center rounded-full'><X width={17} /></button>
            </div>

            <button onClick={() => handleSelect(`/anime/${anime.id}?title=${anime?.title || ''}`)} className="cursor-pointer hover:bg-themeDarker hover:text-blue-400 group flex items-center justify-center gap-2 w-full py-2 bg-themeDarkest border-2 mt-2 border-themeLightDark text-white rounded-lg">
            ...
            <div className="space-y-2 mt-7">
              ...
              <div className="flex items-center gap-3 mt-1">
                <button disabled={epStatus === 0} onClick={handleDecrement} className="text-white bg-themeDarker border border-themeLightDark rounded-full w-7 h-7 flex justify-center items-center cursor-pointer">
                  <Minus width={17} />
                </button>
                <input
                  type="range"
                ...
                />
                <button disabled={epStatus === total_ep} onClick={handleIncrement} className="text-white bg-themeDarker border border-themeLightDark rounded-full w-7 h-7 flex justify-center items-center cursor-pointer">
                  <Plus width={17} />
                </button>
            ...
            <button onClick={handleUpdate} title='update' className='bg-pink-600 hover:bg-pink-500 cursor-pointer w-full justify-center rounded h-8 flex items-center gap-2 text-gray-100'>
                {
