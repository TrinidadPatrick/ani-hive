...
import AnimeForum from "./AnimeForum.jsx";
...
const AnimeOverView = () => {
  ...
  if (!id) return setErrorStatus(404);
  ...
  const InfoRow = ({ label, children }) => (
    <li className="flex justify-start gap-2">
      <h1 className="text-gray-400 min-w-[100px] text-[0.8rem] lg:text-[0.9rem] font-medium w-25">
        {label}
      </h1>
      <h1 className="text-gray-200 line-clamp-1 text-[0.8rem] lg:text-[0.9rem] font-bold">
        {children}
      </h1>
    </li>
  );
  ...
  return (
    <main className="w-full grid grid-cols-13 h-fit relative overflow-hidden sm:p-3">
      <div
        className="absolute inset-0 bg-[url(https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=1920&q=80)] z-0 brightness-40 opacity-70"
        aria-hidden="true"
      />

      {showTrailer && (
        <TrailerPlayer youtubeId={youtubeId} setShowTrailer={setShowTrailer} />
      )}
      {!isLoading && animeInfo ? (
            ...
            <div className="flex-1 h-full  flex flex-col gap-2 px-2">
              ...
              <div className="w-full flex gap-2 my-1">
                <div
                  style={{
                    backgroundImage:
                      "linear-gradient(135deg, #f59f0a33, #c47f081a)",
                    border: "1px solid hsl(38 92% 50% / .3)",
                  }}
                  className="flex gap-2 items-center px-3 rounded-2xl"
                >
                  <Star width={15} className="text-[#f7b23b] fill-[#f7b23b]" />
                  <h1 className="text-[#f7b23b] text-[0.7rem] font-medium mt-0.5">
                    {animeInfo?.score || 0}
                  </h1>
                </div>
                <div className="px-3 bg-themeLightDark rounded-2xl flex items-center">
                  <h1 className="text-white text-[0.7rem] font-medium">
                    {animeInfo?.rating
              ...
              <div className="w-full flex flex-col w gap-2 justify-between">
                <div className="">
                  <h1 className="text-white text-2xl lg:text-4xl font-bold line-clamp-2">
                    {animeInfo?.title_english || animeInfo?.title}
                  </h1>
                </div>
                <div className="flex gap-2 my-2 h-10 ">
                  ...
                  {animeUserStatus ? (
                    <button
                      ...
                      className="bg-pink-600  flex-grow-1 sm:flex-grow-0 flex justify-center capitalize py-2 text-white text-sm h-full px-2 sm:px-5 items-center rounded-lg relative gap-2 cursor-pointer hover:bg-pink-500"
                    >
                    ...
                    <button
                      ...
                      onClick={() => {
                        handleAction("plan_to_watch");
                      }}
                      className="flex gap-2 text-white whitespace-nowrap text-xs md:text-sm w-37 justify-center items-center py-2 bg-pink-600 rounded-lg font-medium cursor-pointer hover:bg-pink-500"
                    >
                      {isUpdating ? (
                        <LoaderV2 width={7} height={7} color={"bg-gray-100"} />
                      ) : (
                  ...
                  <button
                    ...
                    className={`${animeUserStatus === null && "hidden"} flex items-center justify-center text-white whitespace-nowrap text-xs md:text-base w-12 py-2 bg-pink-600 rounded-lg font-medium cursor-pointer hover:bg-pink-500`}
                  >
                    {isDeleting ? (
                      <LoaderV2 width={6} height={6} color={"bg-gray-100"} />
                    ) : (
                      <Trash2 width={19} />
                    )}
                  </button>
          ...
          <div className="w-full h-full  ">
            <div className="w-full h-fit  py-0 px-0 flex flex-col">
              <div className="flex-1 h-fit flex flex-col gap-2">
                ...
                <div className="text-gray-400 text-[0.8rem] md:text-sm lg:text-base bg-themeDarker border border-themeLightDark rounded-lg p-3">
                  {animeInfo?.synopsis?.replace("[Written by MAL Rewrite]", "")}
                </div>
              </div>
            </div>
          </div>
