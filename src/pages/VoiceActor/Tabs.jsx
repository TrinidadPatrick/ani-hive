...
const Tabs = ({personInfo}) => {
    ...
    const [displayLimit, setDisplayLimit] = useState(scrollPosition?.voiceActor ? 100000 : 50)    
    const tabItems = [
        {   
            label: 'Anime', value: 'anime', icon: Film, data: personInfo.anime, 
        ...
        {   
            label: 'Voice Roles', value: 'voices', icon: Mic, data: personInfo.voices, 
            char_mal_id: (item) => item?.character.mal_id, 
            mal_id: (item) => item?.anime?.mal_id,
            title: (item) => item?.anime?.title, 
            ...
            image: (item) => item?.anime?.images?.jpg?.image_url},
        {
            label: 'Manga', value: 'manga', icon: BookOpen, data: personInfo.manga, 
            char_mal_id: (item) => item?.character?.mal_id, 
    ...
    const RenderTabs = React.memo(({tabItem, active, handleClick, index}) => {
        return (
            <button onClick={()=> handleClick(index)} className={`cursor-pointer flex gap-2 flex-1 justify-center py-1 rounded-lg ${active ? 'bg-themeDarker text-white' : 'text-gray-200'}`}>
                <tabItem.icon width={16} />
                <span className='text-xs sm:text-base mt-1 sm:mt-0'>{tabItem.label}</span>
            </button>
        )
    })

    const RenderItems = React.memo(({char_mal_id, mal_id, title, subtitle, image}) => {
        return (
            <div onClick={()=>{setScrollPosition({...scrollPosition, voiceActor: window.pageYOffset});navigate(`/anime/${mal_id}?title=${title}&va=${char_mal_id || 0}`)}} 
            className='flex gap-2 cursor-pointer hover:bg-themeDarker rounded-lg'>
                ...
                <div>
                    <h2 className='text-gray-100 font-medium text-base lg:text-lg line-clamp-3'>{title}</h2>
    ...
    return (
        ...
        <nav className='w-full flex gap-1 sm:gap-2 justify-evenly bg-themeDark p-1 rounded-lg'>
            {
                tabItems.map((tabItem, index) => {
                    return (
                        <RenderTabs handleClick={handleClick} index={index} active={selectedTab == index} key={index} tabItem={tabItem} />
                    )
                })
            }
        </nav>
        ...
        <section className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3'>
            ...
            {
                tabItems[selectedTab].data.slice(0,displayLimit).map((item, index) => {
                    ...
                    return (
                        ...
                        >
                        <RenderItems key={index} char_mal_id={tabItems[selectedTab].char_mal_id(item)} mal_id={tabItems[selectedTab].mal_id(item)} title={tabItems[selectedTab].title(item)} subtitle={tabItems[selectedTab].subtitle(item)} image={tabItems[selectedTab].image(item)} />
                        </motion.div>
                    )
                })
            }
